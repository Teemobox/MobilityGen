...
from omni.ext.mobility_gen.utils.prim_utils import join_prim_path, prim_compute_bbox
...
async def occupancy_map_generate_from_prim_async(
    ...
    prim_path_obj = stage.GetPrimAtPath(prim_path)

    lower_bound, upper_bound, midpoint = prim_compute_bbox(prim_path_obj)
...
def occupancy_map_add_to_stage(
    ...
    # Add mesh
    mesh = UsdGeom.Mesh.Define(stage, join_prim_path(path, "mesh"))
    ...
    # Add material
    material_path = join_prim_path(path, "material")
    material = UsdShade.Material.Define(stage, material_path)
    pbrShader = UsdShade.Shader.Define(stage, join_prim_path(material_path, "shader"))
    ...
    stReader = UsdShade.Shader.Define(stage, join_prim_path(material_path, "st_reader"))
    stReader.CreateIdAttr('UsdPrimvarReader_float2')
    diffuseTextureSampler = UsdShade.Shader.Define(stage, join_prim_path(material_path, "diffuse_texture"))
